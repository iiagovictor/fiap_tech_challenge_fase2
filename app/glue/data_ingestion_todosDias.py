...
def discover_all_finance_keys(s3_client, bucket: str, object_key: str) -> List[str]:
    """
...
def main():
    ...
    s3 = boto3.client("s3")
    keys = discover_all_finance_keys(s3, bucket_name, object_key)
    if not keys:
        print("[INFO] Nenhum finance.parquet encontrado. Encerrando ok."); sys.exit(0)

    # --- cria/alinha a tabela usando o DF ENRIQUECIDO do primeiro arquivo ---
    first_key = keys[0]
    first_body = s3.get_object(Bucket=bucket_name, Key=first_key)["Body"].read()
    df_first = pd.read_parquet(BytesIO(first_body))
    # anexa partições ao primeiro df com base no caminho
    m0 = re.search(r"ano=(\d{4})/mes=(\d{1,2})/dia=(\d{1,2})", first_key)
    if m0:
        df_first["ano"], df_first["mes"], df_first["dia"] = map(int, m0.groups())
    if "ticker" not in df_first.columns: df_first["ticker"] = pd.Series(dtype="string")
    df_first_enriched = compute_enrichments(df_first.copy())  # <<< enriquecido
    ensure_database(refined_db)
    ensure_table_4p_from_df(refined_db, refined_table, refined_path, df_first_enriched)

    # --- processa todos os dias ---
    ...
    print(f"[OK] Partições registradas: {created}/{len(particoes_escritas)}")

    if last_df is not None:
        update_glue_table_schema_from_df(refined_db, refined_table, refined_path, last_df)

    print("[DONE] Concluído (todos os dias).")
